...
const getRecipes = async (req, res, next) => {
  try {
    ...
    const [recipes, total] = await Promise.all([
      Recipe.find(query).skip(offset).limit(parseInt(limit)).sort({ created_at: -1 }),
...
const getRecommendations = async (req, res, next) => {
  try {
    ...
    let ingredientNames = mongoFridge.map((item) => item.ingredient_name.toLowerCase());
    const user = await User.findById(req.user._id);

    let query = {};
    if (ingredientNames.length > 0) {
      query["ingredients.name"] = { $in: ingredientNames.map((name) => new RegExp(name, "i")) };
    }
...
const getByMealType = async (req, res, next) => {
  try {
    ...
    const offset = (parseInt(page) - 1) * parseInt(limit);
    const [recipes, total] = await Promise.all([
      Recipe.find({ meal_type: type }).skip(offset).limit(parseInt(limit)).sort({ created_at: -1 }),
