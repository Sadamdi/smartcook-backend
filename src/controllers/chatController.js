...
const buildUserContext = (user, fridgeItems) => {
  ...
  if (user.allergies && user.allergies.length > 0) parts.push(`Alergi makanan: ${user.allergies.join(", ")}`);
  if (user.medical_history && user.medical_history.length > 0) parts.push(`Riwayat penyakit: ${user.medical_history.join(", ")}`);
  if (user.cooking_styles && user.cooking_styles.length > 0) parts.push(`Gaya masak favorit: ${user.cooking_styles.join(", ")}`);
  if (user.equipment && user.equipment.length > 0) parts.push(`Peralatan dapur: ${user.equipment.join(", ")}`);
  if (fridgeItems.length > 0) {
    parts.push(`Bahan di kulkas: ${fridgeItems.map((i) => `${i.ingredient_name} (${i.quantity} ${i.unit})`).join(", ")}`);
  }
...
const sendMessage = async (req, res, next) => {
  try {
    ...
    const newModelMsg = { role: "model", content: reply, timestamp: new Date() };
    const allMessages = chatDoc ? [...chatDoc.messages, newUserMsg, newModelMsg] : [newUserMsg, newModelMsg];
